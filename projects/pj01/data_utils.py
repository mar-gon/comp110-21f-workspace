...
def corrected_values(majors: dict[str, int]) -> dict[str, int]:
    """Returns dictionary with corrected values."""
    if 'Biostatistics ' in majors:
        majors["Biostatistics"] += majors["Biostatistics "]
        majors.pop("Biostatistics ")
    if 'Environmental Science ' in majors:
        majors["Environmental Science"] += majors["Environmental Science "]
        majors.pop("Environmental Science ")
    if 'Information Science ' in majors:
        majors["Information Science"] += majors["Information Science "]
        majors["Information Science"] += majors["Information science"]
        majors.pop("Information Science ")
        majors.pop("Information science")
    if 'Math ' in majors:
        majors["Math"] += majors["Math "]
        majors.pop("Math ")
    if 'Neuroscience ' in majors:
        majors["Neuroscience"] += majors["Neuroscience "]
        majors.pop("Neuroscience ")
    if 'Psychology' in majors:
        majors["Psychology"] += majors["Psychology "]
        majors.pop("Psychology ")
    if 'Statistics' in majors:
        majors["Statistics"] += majors["Statistics "]
        majors.pop("Statistics ")
    return majors